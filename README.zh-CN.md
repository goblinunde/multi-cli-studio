...
- `API Providers`、`Codex`、`Gemini`、`Kiro` 的统一平台账号管理
...
- 统一的平台中心，覆盖 `API Providers`、`Codex`、`Gemini`、`Kiro`
- 支持 OAuth、手动导入、本地账号导入、账号导出与当前账号切换
- 提供 Codex provider 快切、Gemini 账号注入、Kiro credits / 账号总览
- 桌面端内置全局代理与分平台额度自动刷新配置
...
- 统一承载 API provider 配置与 `Codex`、`Gemini`、`Kiro` 账号中心
- 管理 provider 凭据、平台账号、本地实例与刷新行为
...
- 查看已安装 CLI、本地运行时资源、代理 / 自动刷新设置和环境相关状态
...
桌面端的 Gemini OAuth 流程不再把客户端凭据放进仓库。启动应用前请先设置：

```bash
export MULTI_CLI_STUDIO_GEMINI_OAUTH_CLIENT_ID="你的 Google OAuth Client ID"
export MULTI_CLI_STUDIO_GEMINI_OAUTH_CLIENT_SECRET="你的 Google OAuth Client Secret"
```

如果你不想自行维护 OAuth 凭据，可以改用 `~/.gemini/oauth_creds.json` 本地导入，或者直接使用基于 API Key 的 Gemini 模式。
...
当前 fork 也会把仓库内默认 updater feed 对齐到当前 GitHub 仓库，并且发布初始化 / 自检脚本现在支持识别 `git@github_alias:owner/repo.git` 这类 GitHub SSH 别名远端。
...
- 当前仓库基于 [Austin-Patrician/multi-cli-studio](https://github.com/Austin-Patrician/multi-cli-studio) 继续演进；该上游仓库最初以 MIT 许可发布。
- 平台账号管理方向、发布呈现方式以及许可证表述，也参考了 [jlcodes99/cockpit-tools](https://github.com/jlcodes99/cockpit-tools)。

## 致谢

- 感谢 Austin-Patrician 开源原始 `multi-cli-studio` 项目。
- 感谢 jlcodes99 公开 `cockpit-tools` 项目及相关产品思路。
- 感谢 LinuxDo 社区提供讨论、反馈与测试支持。

## 许可证

本仓库默认采用 [CC BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/deed.zh-hans) 许可协议。

- 允许：个人学习、研究、非商业场景下的使用与修改，前提是保留署名并按相同方式共享。
- 不允许：任何未获单独书面授权的商业使用，包括企业内部商业目的、对外收费服务、付费产品集成和二次分发售卖。
- 商业授权：如需商业使用，请联系当前维护者获取单独书面授权与报价。
- 来自 `Austin-Patrician/multi-cli-studio` 的继承部分，仍在 [LICENSE](./LICENSE) 中保留原始 MIT notice。

更多来源说明和署名信息见 [NOTICE.md](./NOTICE.md)。
